...
레이아웃 규칙:
- cover: 풀화면 사진 + 큰 표지 제목. title은 한 줄로 길게 쓰지 말고 2~3개 덩어리로 자연스럽게 줄바꿈되기 좋게 만든다. 마지막 덩어리는 짧고 강해야 한다. body는 한 문장 이하.
...
- quote: 어두운 사진 위에 중앙 정렬 문장. body에는 앞설명과 맥락을, highlight에는 한 줄로 강조 가능한 짧은 핵심 문구를 넣는다. highlight는 10~22자 정도의 독립적인 문구로 만들고 쉼표나 군더더기를 피한다.
...
- quote 본문은 35~90자 정도로 쓰고, highlight는 한 줄로 강하게 보일 수 있는 짧은 문구 하나로 지정한다.
- list의 items는 각 12~32자 정도, 한 장에 3~4개를 기본으로 한다.
- 어려운 말, AI가 쓴 듯한 상투어, 뻔한 “왜 화제일까?”, 억지 감동, 과도한 느낌표를 피한다.
- 제목과 본문에 마크다운 기호를 넣지 않는다.
- 줄바꿈은 반드시 단어나 조사 중간이 아니라 의미 단위·문장 단위에서만 한다.
- 표지 title은 필요하면 \n을 넣어 2~3줄로 의도적으로 나눈다.
- split/list의 body가 두 문장 이상이면 문장 사이에 \n을 넣는다.
- 화살표(→) 흐름은 한 덩어리의 단어를 쪼개지 말고 화살표 앞뒤에서만 줄을 바꾼다.
  ...
  "slides": [
    {
      ...
      "title":"string",
      "body":"string",
      "highlight":"한 줄 강조용 핵심 문구 또는 빈 문자열",
